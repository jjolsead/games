...
class ActorEvents_13 extends ActorScript
{
	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if((actor.getAnimation() == actor.getAnimation()))
				{
					actor.applyImpulse((Engine.engine.getGameAttribute("PinkX") - actor.getX()), (Engine.engine.getGameAttribute("PinkY") - actor.getY()), .5);
				}
				else
				{
					actor.applyImpulse((Engine.engine.getGameAttribute("PinkX") - actor.getX()), (Engine.engine.getGameAttribute("PinkY") - actor.getY()), -.5);
